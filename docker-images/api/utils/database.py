...
from env import MONGO_URI, MONGO_DATABASE
...
try:
    ...
    metrics.items_total.set(collection.count_documents({}))
...
def create_one_item(item):
    ...
    metrics.database_latency_seconds.labels(
        operation='create_one_item'
        ).observe(time.time() - start_time)

    metrics.items_total.inc(1)
...
def delete_many_items(pattern) -> int:
    ...
    metrics.items_total.dec(result.deleted_count)
